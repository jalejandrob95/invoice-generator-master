...
class InvoiceModal extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      invoiceNumber: "001", // Valor inicial predeterminado
    };
  }

  handleFetch = () => {
    const url = "http://localhost:3003/v1/invoice/create";
    const options = {
      method: "POST",
      headers: {
  ...
  handleFetchGet = () => {
    const url = "http://localhost:3003/v1/invoice/";
    const options = {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
    ...
    fetch(url, options)
      ...
      .then((data) => {
        if (data && data.invoiceNumber) {
          this.setState({ invoiceNumber: data.invoiceNumber });
        }
      })
      .catch((error) => {
        console.log(error);
